...
| [list](./list-command)           | Displays a list of packages from a given source. If no sources are specified, all sources defined in `%AppData%\.dpm\dpm.config.yaml` are used.                                                                            | yes         |
| [mcp](./mcp-command)             | Runs a read-only MCP (Model Context Protocol) server over stdio, so AI coding agents can search DPM packages and inspect Delphi projects.                                                                            | yes         |
...
| [project](./project-command)     | Reports what a project targets - compiler version, platforms - and the DPM packages it references, showing which are top level and which are transitive.                                                              | yes         |
...
| [sources](./sources-command)     | Manages the list of package sources in `%AppData%\.dpm\dpm.config.yaml`.                                                                                                                                                  | yes         |
| [spec](./spec-command)           | Generates a `package.dspec` for a new package. If run in a folder with a `.dproj`, the generated spec is tokenised based on the project.                                                                        | yes         |

...
- If the repo ships only `.pas` source (no `.dpk`/`.dproj`) but you still want
  compiled, IDE-installable packages, add a `package definitions` section so DPM
  generates the project files and compiles them. See
  [package definitions](./creating-packages.md#package-definitions).
